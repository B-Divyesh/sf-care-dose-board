...
function renderMedications(): string {
  ...
  content += '<div class="callout" style="margin-top:24px"><h3>Keep the care plan authoritative</h3><p>Dose Witness never recommends amounts or changes. Update this board only when the person’s established instructions change.</p></div>';
  return content;
}
...
function openMedicationDialog(id?: string): void {
  ...
  dialog.innerHTML = `<form method="dialog" class="dialog-inner" id="medication-form"><div class="dialog-head"><div><p class="eyebrow">Care plan</p><h2>${medication ? 'Edit medication' : 'Add medication'}</h2></div><button class="icon-button" type="button" data-close aria-label="Close dialog">×</button></div>
    ...
    ${medication ? `<div class="field"><label><input style="width:auto;min-height:24px" name="active" type="checkbox" ${medication.active ? 'checked' : ''}/> Active on today’s board</label></div>` : ''}
