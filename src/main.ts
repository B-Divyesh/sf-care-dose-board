...
function renderMedications(): string {
  let content = pageHead('Care plan', 'Medications', 'Copy the name, strength, instructions, and times from the existing care plan.', '<button class="button primary" type="button" data-add-med>＋ Add medication</button>');
...
const metaByView: Record<View, { title: string; description: string }> = {
  ...
  settings: { title: 'Settings — Dose Witness', description: 'Manage the household board name and default caregiver initials.' },
  privacy: { title: 'Privacy — Dose Witness', description: 'Learn what Dose Witness stores on this device.' },
...
function render(options: { focus?: boolean } = {}): void {
  ...
  if (target && (options.focus || location.hash === '#about-art')) {
    ...
    requestAnimationFrame(() => {
      ...
      if (location.hash === '#about-art') {
        const headerHeight = root.querySelector<HTMLElement>('.app-header')?.getBoundingClientRect().height ?? 0;
